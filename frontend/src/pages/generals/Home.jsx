...
export default function Home() {
  ...
  useEffect(() => {
    axios
      .get('https://localhost:5000/api/food', { withCredentials: true })
      .then(res=>{
  ...
  async function likeVideos(item){
    try {
      const res = await axios.post("https://localhost:5000/api/food/like",{foodId: item._id},{withCredentials:true});
      if(res.data.like){
  ...
  async function bookmarkVideo(item){
    try {
      const res = await axios.post("https://localhost:5000/api/food/save",{foodId:item._id},{withCredentials:true});
      if(res.data.save){
